...
:build_share
    ...
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f jar-share
	call %MVN_EXEC% clean package -pl jar-share,jar-share-docker
EXIT /B 0
:build_acs
    docker-compose -f "%COMPOSE_FILE_PATH%" kill jar-acs
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f jar-acs
	call %MVN_EXEC% clean package -pl jar-integration-tests,jar-platform,jar-platform-docker
EXIT /B 0
:tail
...
EXIT /B 0
:prepare-test
    call %MVN_EXEC% verify -DskipTests=true -pl jar-platform,jar-integration-tests,jar-platform-docker
EXIT /B 0
:test
    call %MVN_EXEC% verify -pl jar-platform,jar-integration-tests
EXIT /B 0
:purge
